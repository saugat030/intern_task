...
const Text = ({ data, id }: TextProps) => {
  ...
  return (
    <div className="relative mx-auto h-[768px] w-full xl:h-[528px] 2xl:h-[852px] xl:w-[1248px] 2xl:w-[1504px]">
      ...
      />
      <SearchBar />
      <div className="absolute md:gap-6 gap-2 flex flex-col text-white p-3 bottom-32 md:w-[550px]">
        <h1 className="font-bold md:text-6xl text-3xl">{foundObj?.title}</h1>
        <p className="md:text-xl text-base h-[93px] ps-2 font-medium line-clamp-3">
          {foundObj?.overview}
        </p>
